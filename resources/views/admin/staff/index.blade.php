...
<div class="modal fade text-left defaultSize-modal modal-padding-change" id="inlineForm" tabindex="-1" role="dialog" aria-labelledby="myModalLabel33" aria-hidden="true">
	<div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
		<div class="modal-content">
			...
			<form action="{{url('/')}}/admin/staff/store_staff" method="POST" id="staff_add_edit">
				@csrf
	...
	$(document).ready(function(){
        ...
        $('.validate-frm').click(function(){
			...
			if($('input[name="email"]').val()=="" || $('input[name="email"]').val()=="undefined"){
        		flag = 1;
        		$('input[name="email"]').parent().append('<span class="err-msg">Please enter email</span>');
        	}
			
        	if(flag==0){
        		$('#staff_add_edit').submit();
        	}
        });
	});
			...
			$.ajax({
	            ...
	            success:function(data)
	            {
	            	if(data){
		            	...
		            	$('#staff_add_edit').append('<input type="hidden" name="id" value="'+btoa(data.id)+'" />');
		            	$('#staff_add_edit').attr('action','{{url("/")}}/staff/update');
