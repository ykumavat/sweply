								...
								<div class="breadcrumb-main-bx">      
			                        ...
			                        <div class="breadcrumb-wrapper ">
			                            <ol class="breadcrumb">
			                                ...
			                                <li class="breadcrumb-item active">Channel
			                                </li>
			                            </ol>
			                        </div>
		                        </div>
								...
								<div class="search-action-btn">
									...
									<div class="">
										<button type="button" class="btn btn-primary add-form-btn btn-add-bussiness"  data-toggle="modal" data-target="#inlineForm">											
											<span class="text-nowrap"><span class="table-add-txt">Add Channel</span><span class="table-add-icon"><i class="fal fa-plus"></i></span></span>
										</button>
									</div>
								</div>
							</div>
	...
	<div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h4 class="modal-title" id="myModalLabel33">Add Channel</h4>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
			<form action="{{url('/')}}/admin/create-channel" method="POST" id="channelFrm" enctype="multipart/form-data">
				@csrf
				<div class="modal-body">
					...
					<div class="form-group image-input">
                        <label for="image">Channel image <span class="info-tool-tip"><i class="fas fa-info-circle"></i> <span class="tool-info">Size Required 1920 * 2340</span></span> </label>
                        <div class="custom-file">
                            <input type="file" class="custom-file-input" name="channel_image" id="inputGroupFile01" accept="image/*" />
...
<script type="text/javascript">
	$(document).ready(function(){
	    ...
	    var table = $('.data-table').DataTable({
	        ...
	        serverSide: true,
	        ajax: {
	          url: "{{url('/')}}/admin/channel/load-data",
	          data: function (d) {
	                d.status = $('#status').val(),
	        ...
	        columns: [
	            {data: 'id', name: 'id'},
	            {data: 'channel_image', name: 'channel_image'},
	            {data: 'channel_name', name: 'channel_name'},
	            {data: 'channel_status', name: 'channel_status'},
        ...
        $('.validate-frm').click(function(){
        	$('.err-msg').remove();
        	var flag = 0;
        	if($('input[name="channel_name"]').val()=="" || $('input[name="channel_name"]').val()=="undefined"){
        		flag = 1;
        		$('input[name="channel_name"]').parent().append('<span class="err-msg">Please enter channel name</span>');
        	}
        	if(flag==0){
        		$('#channelFrm').submit();
        	}
			...
			$.ajax({
	            ...
	            success:function(data)
	            {
	            	if(data){
		            	...
		            	$('#channelFrm').attr('action','{{url("/")}}/admin/update-channel');
		            	jQuery(".uploaded-img-main").show();
	            		jQuery("#original_file_display").attr("src",data.channel_image_url);
		            	jQuery(".custom-file-label").text(data.channel_image);
		            }
	            	console.log(data);
	            }
	        });
